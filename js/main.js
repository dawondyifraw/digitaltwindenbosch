...
async function getPlaceName(latitude, longitude) {
    const url = `https://api.tomtom.com/search/2/reverseGeocode/${latitude},${longitude}.json?key=${tomTomApiKey}`;
...
function addCombinedContextMenu(viewer) {
    ...
    document.getElementById('showInfo').addEventListener('click', () => {
        ...
        displayDiv.innerHTML = `<h2>Info</h2><p>Digital Twins Den Bosch v.2.0 in collboration with JADS, 2024. All Rights Reserved.</p><button onclick="document.getElementById('displayDiv').style.display='none'">Close</button>`;
        hideContextMenu();
    });
